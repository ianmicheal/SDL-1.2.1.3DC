...
float triangle_offset = -1.5f;
float square_offset = 1.5f;
...
void DrawGLScene()
{
    ...
    glTranslatef(triangle_offset,0.0f,-6.0f);
    ...
    glLoadIdentity();
    glTranslatef(square_offset,0.0f,-6.0f);
...
{  
    ...
    while (!done) {
        ...
        if (keys[SDLK_LEFT])  triangle_offset -= 0.05f;
        if (keys[SDLK_RIGHT]) triangle_offset += 0.05f;
        if (keys[SDLK_UP])    square_offset += 0.05f;
        if (keys[SDLK_DOWN])  square_offset -= 0.05f;
